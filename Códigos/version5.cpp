...
public:                                                         //V  Se las dimensiopnes de la carta
    ...
    Carta(Color color, string valor, const sf::Font* font) : enJuego(true), color(color), valor(valor), font(font), numero(*font, valor) {
...
public:
    Jugador() : mano(NULL), manoReserva(NULL) {}
    Jugador(vector<Carta*>& mano, vector<Carta*> manoReserva) : mano(mano), manoReserva(manoReserva) {
    }

    void operator+(Carta& carta) {
...
int main() {
    ...
    jugador1 + mesa.getCarta(2, 9); //Aqui se asigna la carta a la mano del jugador, usando el operador sobrecargado
    Carta carta1 = mesa.getCarta(3, 12);
    carta1.setPosition(sf::Vector2f(760.f, 655.f));


    mesa.getCarta(1, 0).setPosition(sf::Vector2f(600.f, 655.f));


    mesa.getCarta(0, 11).setPosition(sf::Vector2f(460.f, 655.f));
    RenderWindow window(VideoMode(Vector2u(1400, 900)), "4 amigos");
    //Aqui se asignan las cartas a la mano del jugador, usando el operador sobrecargado

    mesa.getCarta(2, 10).setPosition(sf::Vector2f(320.f, 655.f));


    jugador1.getCarta(0).setPosition(sf::Vector2f(200.f, 655.f));
    ...
    while (window.isOpen()) {
        ...
        window.draw(mesa);
        window.draw(carta1);
        window.draw(mesa.getCarta(1, 0));
        window.draw(mesa.getCarta(0, 11));
        window.draw(mesa.getCarta(2, 10));
        window.draw(jugador1.getCarta(0));
        window.display();
